...
function buildReminderWhatsAppLink(date, slotKey, name, phone, address) {
  const isHalf = slotKey.endsWith("-a") || slotKey.endsWith("-b");
  const dayName = HEBREW_DAYS[new Date(date + "T00:00:00").getDay()];
  const isShinkin = address && address.includes("שינקין") && address.includes("62") && address.includes("גבעתיים");

  const locationLines = isShinkin
    ? `ניפגש בשינקין 62, גבעתיים (לא תל אביב!)
שער ברזל שחור, הכניסה הימנית (זאת עם הדלת)
קומה אחרונה, דירה 5`
    : `ניפגש ב${address || "המקום שסוכם"}`;

  const priceLines = isHalf
    ? `תספורת עולה 220₪, אפשר באפליקציות או במזומן.
(נרשמת לחצי תור: 150₪)`
    : `תספורת עולה 220₪, אפשר באפליקציות או במזומן.`;

  const message = `היוש! זוהי תזכורת לתספורת שלך ביום ${dayName}!
${locationLines}
${priceLines}
