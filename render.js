...
function render() {
  ...
  if (state.mode === "admin") {
    ...
    app.querySelectorAll("[data-mark-reminder-sent]").forEach((el) =>
      el.addEventListener("click", () => {
        const [d, slotKey] = el.getAttribute("data-mark-reminder-sent").split("|");
        markReminderSent(d, slotKey);
      }));
    ...
    app.querySelectorAll("[data-edit-booking]").forEach((el) =>
      el.addEventListener("click", (e) => {
        e.stopPropagation();
      ...
      ${activeUpcoming.length === 0 ? `
        ...
        <div class="space-y-2">
          ${activeUpcoming.map((date) => {
            ...
            const availabilityText = parts.length > 0 ? `${parts.join(" ו-")} פנויים` : "כל התורים תפוסים";
            return `
